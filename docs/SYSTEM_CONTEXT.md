...
- 分为两个一级模块：设置、表管理。
- 设置模块目前提供全局“显示右侧阅读地图”开关。
- 表管理模块查看索引概览和 `sites/pages/progress/settings` 视图。
- 支持删除站点索引、清空单站点 progress、清空全部 progress。
- 不支持直接编辑表记录，避免破坏数据一致性。
...
- 一级模块为设置和表管理。
- 设置页用卡片和开关管理插件偏好。
- 表管理页用索引列表、顶部 tab 和表格查看 `sites/pages/progress/settings`。
- 危险操作集中在 danger zone，并需要确认。

页面内：

- 左侧导航顶部展示总进度条和百分比。
- 每条已索引目录链接后展示小号百分比 badge。
- 右侧阅读地图展示当前页面已读区间和当前视口位置，可在 options 设置页全局关闭。
- 视觉应轻量，不能干扰原文档阅读。
